...
function activate(context) {
    ...
    const apiUrl = config.get('apiUrl', 'https://api.typing-master-for-coding.com');
    const practiceMode = config.get('practiceMode', 'timed');
    const duration = config.get('duration', 3);
    const showRealTimeMetrics = config.get('showRealTimeMetrics', true);
    const autoSaveResults = config.get('autoSaveResults', true);
    // Register commands
    ...
    context.subscriptions.push(startPracticeCommand, practiceCurrentFileCommand, practiceSelectionCommand, showLeaderboardCommand, shareToGitHubCommand, syncWithGitHubCommand);
...
async function startPracticeSession(context, apiUrl, practiceMode, duration, showRealTimeMetrics, autoSaveResults) {
    try {
        ...
        const response = await axios_1.default.post(`${apiUrl}/api/v1/integrations/embedded/session`, {
        ...
        // Show practice panel
        const panel = vscode.window.createWebviewPanel('typingPractice', 'Typing Practice', vscode.ViewColumn.One, {
            enableScripts: true,
        ...
        panel.webview.onDidReceiveMessage(async (message) => {
            switch (message.command) {
                case 'practiceCompleted':
                    ...
                    vscode.window.showInformationMessage(`Practice completed! Score: ${message.results.composite_score}`);
                    break;
                case 'practiceError':
...
async function practiceCurrentFile(context, apiUrl, practiceMode, duration, showRealTimeMetrics, autoSaveResults) {
    ...
    try {
        ...
        const { session_id, embed_url } = response.data;
        const panel = vscode.window.createWebviewPanel('typingPractice', 'Typing Practice - Current File', vscode.ViewColumn.One, {
            enableScripts: true,
            retainContextWhenHidden: true
        });
        panel.webview.html = getPracticeWebviewContent(embed_url, session_id, showRealTimeMetrics);
...
async function practiceSelection(context, apiUrl, practiceMode, duration, showRealTimeMetrics, autoSaveResults) {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        vscode.window.showWarningMessage('No active editor found');
        return;
    ...
    try {
        ...
        const { session_id, embed_url } = response.data;
        const panel = vscode.window.createWebviewPanel('typingPractice', 'Typing Practice - Selection', vscode.ViewColumn.One, {
            enableScripts: true,
            retainContextWhenHidden: true
        });
        panel.webview.html = getPracticeWebviewContent(embed_url, session_id, showRealTimeMetrics);
...
async function showLeaderboard(context, apiUrl) {
    try {
        const response = await axios_1.default.get(`${apiUrl}/api/v1/leaderboard`);
        const leaderboard = response.data;
        const panel = vscode.window.createWebviewPanel('leaderboard', 'Typing Master Leaderboard', vscode.ViewColumn.One, {
            enableScripts: true,
            retainContextWhenHidden: true
        });
...
async function savePracticeResults(results, apiUrl) {
    try {
        await axios_1.default.post(`${apiUrl}/api/v1/sessions/finalize`, results);
    }
    catch (error) {
        console.error('Failed to save practice results:', error);
    }
}
function detectLanguageFromDocument(document) {
...
function getPracticeWebviewContent(embedUrl, sessionId, showRealTimeMetrics) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
function getLeaderboardWebviewContent(leaderboard) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
async function shareToGitHub(context, apiUrl) {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        vscode.window.showWarningMessage('No active editor found');
        return;
    ...
    try {
        ...
        await vscode.window.withProgress({
            ...
            const response = await axios_1.default.post(`${apiUrl}/api/v1/integrations/github/share`, {
...
async function syncWithGitHub(context, apiUrl) {
    try {
        const config = vscode.workspace.getConfiguration('typing-master');
        let githubToken = config.get('githubToken');
        if (!githubToken) {
        ...
        await vscode.window.withProgress({
            ...
            const response = await axios_1.default.get(`${apiUrl}/api/v1/integrations/github/repos`, {
                headers: {
            ...
            const repoNames = repos.map((repo) => `${repo.full_name} (${repo.language || 'Unknown'})`);
            const selectedRepo = await vscode.window.showQuickPick(repoNames, {
                placeHolder: 'Select a repository to sync snippets from'
            });
            if (!selectedRepo)
                return;
            const repo = repos.find((r) => `${r.full_name} (${r.language || 'Unknown'})` === selectedRepo);
            progress.report({ increment: 100, message: 'Complete!' });
            vscode.window.showInformationMessage(`Synced with ${repo.full_name}. Found ${repo.snippet_count || 0} typing practice snippets.`, 'View Snippets').then(selection => {
