...
func (s *WorkflowEmbeddingService) CreateEmbed(ctx context.Context, req *EmbedRequest) (*EmbedResponse, error) {
	...
	snippetKey, err := s.db.Client.Put(ctx, tempSnippet)
	if err != nil {
	...
	// Save session
	sessionKey, err := s.db.Client.Put(ctx, session)
	if err != nil {
...
func (s *WorkflowEmbeddingService) GetEmbed(ctx context.Context, embedID string) (*EmbedResponse, error) {
	// Get session
	sessionKey := s.db.Client.NameKey("Session", embedID)
	...
	// Get snippet
	snippetKey := s.db.Client.NameKey("Snippet", session.SnippetID)
