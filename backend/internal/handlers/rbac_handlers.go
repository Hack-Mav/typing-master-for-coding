...
func CreateRole(db *database.DatastoreClient) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		role, err := rbacService.CreateRole(ctx, req, userID.(string))
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
...
func UpdateRole(db *database.DatastoreClient) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		err := rbacService.UpdateRole(ctx, roleID, req)
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
...
func CreatePermission(db *database.DatastoreClient) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		permission, err := rbacService.CreatePermission(ctx, req, userID.(string))
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
...
func AssignRole(db *database.DatastoreClient) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		userRole, err := rbacService.AssignRole(ctx, req, assignedBy.(string))
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
			return
		}
