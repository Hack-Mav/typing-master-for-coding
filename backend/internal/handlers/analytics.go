...
// GetABTests returns available A/B tests
func GetABTests(db *database.DatastoreClient) gin.HandlerFunc {
	return func(c *gin.Context) {
		status := c.Query("status")

		tests, err := database.GetABTests(db, status)
		if err != nil {
...
func GetUserABTestAssignment(db *database.DatastoreClient) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			...
			if !isUserInTargetAudience(userID, test.TargetAudience) {
				c.JSON(http.StatusForbidden, gin.H{"error": "User not in target audience"})
				return
			}

			// Assign variant based on traffic split
			variantID := assignVariant(test.TrafficSplit)
...
// CreateABTest creates a new A/B test (for admins)
...
// UpdateABTest updates an A/B test (for admins)
func UpdateABTest(db *database.DatastoreClient) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var req struct {
			...
			TrafficSplit   []models.TrafficSplit `json:"traffic_split"`
		...
		if len(req.TrafficSplit) > 0 {
			test.TrafficSplit = req.TrafficSplit
		}
		if len(req.SuccessMetrics) > 0 {
			test.SuccessMetrics = req.SuccessMetrics
		}
...
// GetABTestResults returns A/B test results (for admins)
