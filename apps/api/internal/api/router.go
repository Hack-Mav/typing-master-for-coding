...
func SetupRouter(db *database.DatastoreClient, cacheClient *cache.InMemoryCache, cfg *config.Config, logger *logging.Logger, errorHandler *errors.ErrorHandler) *gin.Engine {
	...
	router.POST("/forums/:id/posts", handlers.CreateForumPost(db))
	router.GET("/forums/:id/posts", handlers.GetForumPosts(db))
	...
	v1 := router.Group("/api/v1")
	{
		...
		adminOnly.Use(middleware.AdminAuthMiddleware(db))
		{
			...
			// adminOnly.PUT("/support/tickets/:id", handlers.UpdateSupportTicket(db)) // Not implemented
			...
			// adminOnly.POST("/forums", handlers.CreateForum(db)) // Not implemented
			// adminOnly.PUT("/forums/:id", handlers.UpdateForum(db)) // Not implemented
			// adminOnly.DELETE("/forums/:id", handlers.DeleteForum(db)) // Not implemented
