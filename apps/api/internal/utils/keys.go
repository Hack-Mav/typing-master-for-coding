...
func GenerateEntityKey(kind, id string) *database.Key {
	return database.NameKey(kind, id, nil)
}

// GenerateNewEntityKey creates a datastore key for a new entity with a composite ID
// Format: {parentID}_{timestamp}
func GenerateNewEntityKey(kind, parentID string) *database.Key {
	return database.NameKey(kind, fmt.Sprintf("%s_%d", parentID, time.Now().Unix()), nil)
}

// GenerateSessionKey creates a datastore key for a session
// Format: session_{userID}_{timestamp}
func GenerateSessionKey(userID string) *database.Key {
	return database.NameKey("Session", fmt.Sprintf("session_%s_%d", userID, time.Now().UnixNano()), nil)
}

// GenerateEventKey creates a datastore key for a session event
// Format: {sessionID}_event_{timestamp}_{index}
func GenerateEventKey(sessionID string, index int64) *database.Key {
	return database.NameKey("SessionEvent", fmt.Sprintf("%s_event_%d_%d", sessionID, time.Now().UnixNano(), index), nil)
}

// GenerateContentVersionKey creates a datastore key for a content version
// Format: {contentType}_{contentID}_{version}
func GenerateContentVersionKey(contentType, contentID string, version int) *database.Key {
	return database.NameKey("ContentVersion", fmt.Sprintf("%s_%s_%d", contentType, contentID, version), nil)
}

// GenerateValidationKey creates a datastore key for content validation
// Format: {contentType}_{contentID}_{timestamp}
func GenerateValidationKey(contentType, contentID string) *database.Key {
	return database.NameKey("ContentValidation", fmt.Sprintf("%s_%s_%d", contentType, contentID, time.Now().Unix()), nil)
}
